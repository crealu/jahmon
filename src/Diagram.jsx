...
export const Diagram = (props) => {
  ...
  // const extractIds = useMemo((ids) => {
  //   console.log(ids);
  //   return ids.replaceAll('s', '').replaceAll('f', '').split(',').reverse();
  // }, []);

  const notes = useMemo((noteid) => {
    // let ids = extractIds(step.noteids.join());
  ...
  const setClass = useCallback((stringNumber) => {
        ...
        {strings.map((string, sn) => {
          return (
            <div className="diagram-string" key={sn}>
              ...
              {frets.map((fret, fn) => {
                return (
                  <div className={setClass(sn)} key={fn}>
